...
struct shared_info {
	/* AMR value the parent expectes to read in the core file. */
	unsigned long amr;
			   ...
			   pid_t pid)
{
	...
	return buf.st_mtime >= info->core_time ? buf.st_size : 0;
}
			   ...
			   off_t core_size)
{
	...
	amr = *((unsigned long *) note);

	printf("%-30s AMR: %016lx\n", core_read_running, amr);

	FAIL_IF(amr != info->amr);
